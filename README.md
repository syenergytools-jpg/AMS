...
- **Support** — employees raise a complaint (attendance, salary, leave, or other) from a "Support" tab and track its status; admins see every complaint attributed to the raising employee from their own "Support" tab, with a banner on the admin overview when any are open, and resolve one with an optional reply the employee then sees against that complaint.
- **Notifications** — a bell in the sidebar alerts admins when leave is requested or a complaint is raised, and alerts the employee when either is resolved.
